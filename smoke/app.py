...
from .resize import ReversiblePadding
...
class SmokeSession():
    ...
    def detect(self, frame_id: int, frame_image: Image.Image) -> torch.Tensor:       
        ...
        reversible_padding = ReversiblePadding(frame_image, (int(kitti_height), int(kitti_width)))
        padded_frame_image = reversible_padding.get_padded_image()
        ...
        center = np.array([i / 2 for i in padded_frame_image.size], dtype=np.float32)
        size = np.array([i for i in padded_frame_image.size], dtype=np.float32)

        assert size[0] == kitti_width
        assert size[1] == kitti_height
        ...
        padded_frame_image = padded_frame_image.transform(
            (input_width, input_height),
            method=Image.AFFINE,
        ...
        frame_tensor, target = transforms(padded_frame_image, target)
...
if __name__ == "__main__":
    ...
    K_3x4 = get_camera_intrinsics(image_height=image.size[1], image_width=image.size[0])
    K_3x3 = K_3x4[:3, :3]
